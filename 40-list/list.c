...
static void mylist_add(int n)
{
	...
	pr_info("mylist: %d is added to the head\n", n);
}
...
static void mylist_del_head(void)
{
	struct mylist_entry *e =
	    list_first_entry(&mylist, struct mylist_entry, list);
	int n = e->n;
	list_del(&e->list);
	kfree(e);
	pr_info("mylist: %d is deleted from the head\n", n);
}
